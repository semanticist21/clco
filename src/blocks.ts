...
export interface TextBlock {
  type: "text"
  text: string
  /** Copilot's prompt-cache marker rides on the block Claude Code sends. */
...
function placeholder(blockType: string, detail?: string): Classified {
  return {
    kind: "unsupported",
    blockType,
    text: `[clco: a ${blockType} block was not forwarded - this model's dialect has no equivalent${
      detail ? `, ${detail}` : ""
    }]`,
  }
}
...
function classifyKnown(block: unknown, type: KnownBlockType): Classified {
  switch (type) {
    ...
    // every turn - the upstream redacted it, not clco.
...
/** The block types in a body that clco could not forward, for diagnostics. */
